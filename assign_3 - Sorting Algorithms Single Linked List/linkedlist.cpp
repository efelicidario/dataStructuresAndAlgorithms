...
int LinkedList::length() const{
    ...
    while (curNode != nullptr) {   // iterate through list
        count ++;
        curNode = curNode->next;
    }
...
Node* LinkedList::getRear() const {
    return this->rear;
}

int LinkedList::getPos(Node* val) const {
    Node* temp = this->front;
    int position = 0;   
    while (temp != val) {
        temp = temp->next;      // iterate through the list,
        position++;             // increment through each position
    }
    return position;
}
